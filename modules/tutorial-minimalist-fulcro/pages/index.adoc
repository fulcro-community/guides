...
(_ToC: <<Component's props,props>> | <<Component's `:query`,`:query`>> | <<Component's `:ident`,`:ident`>> | <<Component's `:initial-state`,`:initial-state`>> | <<Rendering DOM: the body of a component,body>>_)
