...
This tutorial expects that you have gone through xref:tutorial-eql-pathom-overview:index.adoc[*an EQL Tutorial*] and are familiar with EQL, queries, joins. It will not work without that knowledge. You also need to have https://blog.wsscode.com/pathom/v2/pathom/2.2.0/introduction.html[an idea about *Pathom v2*] and {url-pathom-resolvers}[Pathom resolvers] (global, ident, parameters). (But you don't need to care about Pathom readers.)
