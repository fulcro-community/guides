...
(defsc Person [_ props]
  ...
  ...)
```
<1> Use `:will-enter`, which is called when it is decided to route to the target
...
<3> Return `route-deferred` with the ident and a completion function that will be executed shortly by Fulcro, and which needs to eventually trigger the `dr/target-ready` post-mutation to tell Fulcro that the component is ready. Here the completion function does load the data and uses ``load!``'s `:post-mutation` and `:post-mutation-params` to make sure that `target-ready` is transacted after the load.
====

==== On router timeouts and errors

// TODO: + handling err, "loading..."
