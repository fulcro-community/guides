...
A component can also specify `:initial-state (fn [params] <some data matching the expected props>)` to declare data that should be inserted into the client DB when the application is mounted for the first time. You can use it to define the state of the application before any data is loaded from the server-side.
...
<6> As demonstrated, we submit a mutation for processing using `comp/transact!` and passing in the params. We can call the mutation as a function, which will simply return its name as a symbol (and the params map)
